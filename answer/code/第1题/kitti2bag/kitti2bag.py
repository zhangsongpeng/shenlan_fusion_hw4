...
def save_velo_data(bag, kitti, velo_frame_id, topic, type):
    ...
    for dt, filename in bar(iterable):
        ...
        if type == 'sync':
            ...
            scan = (np.fromfile(velo_filename, dtype=np.float32)).reshape(-1, 4)
            # scan = np.genfromtxt(velo_filename, delimiter=' ').reshape(-1, 4)
...
def run_kitti2bag():
    ...
    bag = rosbag.Bag("kitti_{}_drive_{}_{}.bag".format(args.date, args.drive, args.kitti_type), 'w', compression=compression)
    kitti = raw(args.dir, args.date, args.drive, args.kitti_type)
    print kitti.data_path
    ...
    try:
        ...
        util = utils.read_calib_file(os.path.join(kitti.calib_path, 'calib_cam_to_cam.txt'))
        ...
        save_velo_data(bag, kitti, velo_frame_id, velo_topic, args.kitti_type)
